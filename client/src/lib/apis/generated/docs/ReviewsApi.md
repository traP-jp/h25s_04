...
```typescript
import { ReviewsApi, Configuration } from './api'
...
```typescript
import { ReviewsApi, Configuration, ReviewCreate } from './api'
...
const { status, data } = await apiInstance.reviewsGet(sortBy, page, limit)
```
...
const { status, data } = await apiInstance.reviewsReviewIdDelete(reviewId)
```
...
const { status, data } = await apiInstance.reviewsReviewIdGet(reviewId)
```
...
```typescript
import { ReviewsApi, Configuration, ReviewUpdate } from './api'
